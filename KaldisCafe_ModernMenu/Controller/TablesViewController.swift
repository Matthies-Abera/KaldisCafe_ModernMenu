...
class TablesViewController: UIViewController {
    ...
    func showOpenTableAlert(_ sender: UIButton) {
        ...
        alert.addAction(UIAlertAction(title: "Yes", style: UIAlertAction.Style.default, handler: { action in
            ...
            if let safeTables = self.tables {
                ...
                let newDocket = Docket(docketNumber: safeTables.currentDocketNumber, tableNumber: sender.tag, docketItems: [], totalAmount: 0.00, timeClosed: nil, dayClosed: nil, menu: self.menu!)
                
                // assigning docket to table
                safeTables.tables[sender.tag - 1].docket = newDocket
                
                // Incrementing current docket number
                safeTables.currentDocketNumber += 1
